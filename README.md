...
- [leetcode](#leetcode)
  ...
  - [动态规划](#动态规划)
    ...
    - [354.俄罗斯套娃信封问题（二维的最长递增子序列）](#354俄罗斯套娃信封问题二维的最长递增子序列)
...
```java
/**
 * 将初始的H指数设置为0，然后将引用次数排序，对排序后的数组从大到小遍历
 * 如果当前 H 指数为h，并且在遍历过程中找到当前值 citations[i]>h，则说明我们找到了一篇被引用了至少
 * h+1 次的论文，所以将现有的 h 值加 1。继续遍历直到 h 无法继续增大。最后返回 h 作为最终答案
 */
class Solution {
...
class Solution {
    public int lengthOfLIS(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            //[0,i-1]区间内，如果存在比nums[i]小的数据，就能构成一个递增子序列，+1
            for (int j = 0; j < i; j++) {
                if(nums[i] > nums[j]) {
...
```java
/**
 * 这道题目其实是最长递增子序列的一个变种，因为每次合法的嵌套是大的套小的，
 * 相当于在二维平面中找一个最长递增的子序列，其长度就是最多能嵌套的信封个数
 *
 * 先计算面积，通过面积求最长递增子序列？
 *    不行的，比如1 * 10 大于 3 * 3，当时前者不能装下后者
 *
 * 数学中经常使用控制变量法，我们可以先固定宽度，先对宽度 w 进行升序排序，如果遇到 w 相同的情况，则按照高度 h 降序排序；
 * 之后把所有的 h 作为一个数组，在这个数组上计算 LIS 的长度就是答案。
 *
 * 按照宽度申请需，为什么要对高度 h 进行将序排列呢？
 *  比如排序后的结果为[(1,2),(1,2),(1,3)]，按高度h计算最长升序子序列长度，结果将是：3。但是实际上因为宽度相同
 *  不存在一个信封可以装下另一个信封。
 *
 * 因此，我们要保证每一个 w 值，至多只能选择一个信封。所以 w 值相同的信封，按照 h 将其将序，保证在 w 值相同的情况
 * 下，选不出一个满足条件的信封（因为是降序，不存在后一个信封能将前一个信封装下）。
 *
 * 所以，最后只需要遍历计算出 h 的最长递增子序列长度即可。
 */
class Solution {
